...
class Segmentation:
    ...
    # def save_json(self, filename, json_save_path, category_id, area):
    ...
    #     annotation = [annotation for annotation in self.annotations if annotation["category_id"] == category_id and annotation['area'] == area]
    #     myjson_ = {
    ...
    def save_json(self, filename, category_id, area):
        json_name = "{}.json".format(filename)
        self.myjson["images"][0]["file_name"] = filename
        category = [category for category in self.categories if category["id"] == category_id]
        annotation = [annotation for annotation in self.annotations if annotation["category_id"] == category_id and annotation['area'] == area]
        ...
        self.json_dict[json_name] = myjson_
        
    # def save(self, json_save_path,category_id, imagename ,image_save_path, result_img, area):
    #     if not os.path.isdir(image_save_path):
    #         os.mkdir(image_save_path)
    #     path_list = imagename.split('.')
    #     category = self.categoryname[category_id]
    #     filename = "{}_{}_{}.{}".format(path_list[0], category,self.imagenumber, path_list[1])
    #     filepath = "{}/{}".format(image_save_path,filename)
    #     cv.imwrite(filepath, result_img)
    #     self.save_json(filename, json_save_path, category_id, area)

    def save(self,category_id, imagename ,result_img, area):
        path_list = imagename.split('.')
        category = self.categoryname[category_id]
        filename = "{}_{}_{}.{}".format(path_list[0], category,self.imagenumber, path_list[1])
        self.image_dict[filename] = result_img
        self.save_json(filename, category_id, area)

    # def segment(self,imagepath,json_save_path,image_save_path):
    #     for annotation in self.annotations:
    #         seg, _, category_id, area = annotation['segmentation'], annotation['image_id'], annotation['category_id'], annotation['area']
    #         if category_id == 118 or category_id == 121 or category_id == 119 or category_id == 120 or category_id == 123 or category_id == 122:
    ...
    #             self.save(json_save_path, category_id, imagename,image_save_path,result_img, area)
    #             self.imagenumber += 1
    ...
    def segment(self,image):
        ...
        for annotation in self.annotations:
            ...
            if category_id == 118 or category_id == 121 or category_id == 119 or category_id == 120 or category_id == 123 or category_id == 122:
                ...
                contours = self.return_contours(seg)
                result_img = self.mask_image(img, contours)
                self.save(category_id, imagename,result_img, area)
                self.imagenumber += 1
